		...
		<el-aside width="300px" v-loading="showDicloading">
			<el-container>
					...
					<el-tree ref="dic" class="menu" node-key="id" :data="dicList" :props="dicProps" :highlight-current="true" :expand-on-click-node="false" :filter-node-method="dicFilterNode" @node-click="dicClick">
						<template #default="{node, data}">
							<span class="custom-tree-node">
								...
								<span class="do">
									<el-button-group>
										<el-button icon="el-icon-edit" size="small" v-auth="'sys:dict:update'" @click.stop="dicEdit(data)"></el-button>
										<el-button icon="el-icon-delete" size="small" v-auth="'sys:dict:delete'" @click.stop="dicDel(node, data)"></el-button>
									</el-button-group>
								</span>
							</span>
						</template>
				...
				<el-footer style="height:51px;">
					<el-button type="primary" size="small" icon="el-icon-plus" style="width: 100%;" v-auth="'sys:dict:save'" @click="addDic">字典分类</el-button>
				</el-footer>
			</el-container>
		</el-aside>
		<el-container class="is-vertical">
			...
			<el-main class="nopadding">
				<scTable ref="table" :apiObj="listApi" row-key="id" :params="listApiParams" @selection-change="selectionChange" stripe :paginationLayout="'prev, pager, next'">
					...
					<el-table-column label="键值" prop="key" width="200"></el-table-column>
					<el-table-column label="是否有效" prop="yx" width="200">
						...
						<template #default="scope">
							<el-button-group>
								...
								<el-popconfirm title="确定删除吗？" @confirm="table_del(scope.row, scope.$index)">
									<template #reference>
										<el-button text type="primary"  v-auth="'sys:dic:delete'" size="small">删除</el-button>
									</template>
								</el-popconfirm>
							</el-button-group>
						</template>
		...
		methods: {
			...
			dicDel(node, data){
				this.$confirm(`确定删除 ${data.name} 项吗？`, '提示', {
					...
					var res = await this.$API.system.dic.deletDicType.get({id:data.id});
					if (res.code == 200) {
			...
			async table_del(row, index){
				...
				var res = await this.$API.system.dic.deletDicData.get(reqData);
				if(res.code == 200){
